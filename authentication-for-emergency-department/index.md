...
TODO: merge in ADR template from Chris Collis ASAP; this will adjust all the formatting, create the right template headlines, etc.
...
We are investigating many options.
...
There are commerical options for rapid disinfectable screens, including with dedicated phyiscal buttons to lock the screen, so wiping the screen doesn't accidentally cause any interface touch events.
